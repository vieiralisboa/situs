...
class Docs_Controller {
    ...
    public function get() {
        ...
        Router::route('/docs/bar/templates/:template', function($request) {
            $file = Router::$controller_config->htdocs."/sites/ze/js/bar/templates/".$request->data['template'];
            if(!file_exists($file)) Util::quit(404);
            return Util::serve($file);
        });

        Router::route('/docs/bar/css/:sheet', function($request) {
            $file = Router::$controller_config->htdocs."/sites/ze/js/bar/css/".$request->data['sheet'];
            if(!file_exists($file)) Util::quit(404);
            return Util::download($file);
        });

        Router::route('/docs/bar/:file', function($request) {
            $file = Router::$controller_config->htdocs."/sites/ze/js/bar/js/".$request->data['file'];
            if(!file_exists($file)) Util::quit(404);
            return Util::download($file);
        });
