...
class MyTV52_Controller {
    ...
    public function get($request) {   
        ...
        $path = "/shares/Public/Shared Videos/tv-shows";

        // valid requests contain 
        if(count($request->uri) < 3) return; 

        
        if($request->uri[1] == 'sent')
        {
            $json =  $path . "/New/SENT.situs.json";
            return file_exists($json)? json_decode(file_get_contents($json)): array();
        }
        ...
        if($request->uri[1] == 'dir')
        {
            $dir = $request->uri[2];
            $path .= "/".$dir;
            $url = "http://situs.no-ip.org:8080/myTV/show/";//"http://guest:guest@95.136.117.243:8080/myTV/show/";
            $videos = array();
            foreach(scandir($path) as $video)
            {
                ...
                if(preg_match('/\.mp4$/', $video)) 
                {
                    ...
                    $show = array(
                        ...
                        //TODO insert users user:pw
                        'mp4' => "show/" . $dir."/".$info['filename']
                        //'mp4' => $dir."/".$info['filename']
                    );
