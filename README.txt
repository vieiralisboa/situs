...
3. HTACCESS (see Annex A: Install Apache Server)
...
A2. Apache
	A2.1. Install Apache 2.4 Web Server
		a) Extract httpd-2.4.x-win64-VC11.zip to c:\Apache24

		b) Uncomment rewrite and headers modules in c:/Apache24/conf/httpd.conf

			LoadModule rewrite_module modules/mod_rewrite.so
			LoadModule headers_module modules/mod_headers.so

		c) Edit httpd.conf to allow directives in .htaccess files

			<Directory "c:/situs/public">
				AllowOverride All
			</Directory>

		d) Register the Apache Service
...
A3. PHP (Windows)
	A3.1. Install PHP
		a) Extract php-5.6.x-Win32-VC11-x64.zip to c:/PHP
